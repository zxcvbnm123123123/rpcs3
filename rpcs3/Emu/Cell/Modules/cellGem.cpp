...
namespace tracker
{
	...
	void enable(gem_t* gem, u32 gem_num)
	{
	...
	namespace map
	{
		static bool gem_state_tracker(const gem_t::gem_controller& controller, vm::ptr<CellGemState>& gem_state)
		{
			if (!gem_state)
			{
				return false;
			...
			const auto distance = psmove_tracker_distance_from_radius(tracker, r) * 10; // cm->mm Multiplie distance x3
			float fx1 = fx * 30, fy1 = -fy * 30, z3 = -z * 30;
			...
			gem_state->handle_pos[0] = fx1;
			gem_state->handle_pos[1] = fy1;
		...
		static bool psmove_input_to_gem_image_state(const gem_t::gem_controller& controller, vm::ptr<CellGemImageState>& gem_image_state)
		{
			if (!gem_image_state)
			{
				return false;
			...
			psmove_tracker_get_size(tracker, &width, &height);			
			const auto age = psmove_tracker_get_position(tracker, handle, &x, &y, &r) * 1000;  // ms -> us
			const auto distance = psmove_tracker_distance_from_radius(tracker, r) * 20;
			...
			gem_image_state->projectionx = x * distance;
			gem_image_state->projectiony = y * distance;
			gem_image_state->distance = distance;

			//cellGem.fatal("u: %+01.3f, v: %+01.3f, r: %+01.3f, projX: %+01.3f, projY: %+01.3f, dist: %+01.3f}", gem_image_state->u, gem_image_state->v, gem_image_state->r, gem_image_state->projectionx, gem_image_state->projectiony, gem_image_state->distance);
...
static bool ds3_input_to_pad(const u32 port_no, be_t<u16>& digital_buttons, be_t<u16>& analog_t)
{
	...
	if (pad->m_press_R1)
		cellGem.fatal("Move fake pressed"),
...
static bool psmove_input_to_gem(const gem_t::gem_controller& controller, vm::ptr<CellGemState>& gem_state)
{
	...
	float w, x, y, z;
	float temp = psmove_get_temperature(handle);
...
s32 cellGemGetImageState(u32 gem_num, vm::ptr<CellGemImageState> gem_image_state)
{
	...
	else if (g_cfg.io.move == move_handler::move)
	{
		...
		move::psmoveapi::tracker::map::psmove_input_to_gem_image_state(handle, gem_image_state);
	}
...
s32 cellGemGetInertialState(u32 gem_num, u32 state_flag, u64 timestamp, vm::ptr<CellGemInertialState> inertial_state)
{
	...
	if (g_cfg.io.move == move_handler::fake)
	{
		if (g_cfg.io.mouse == mouse_handler::basic)
		{ 
			move::map::mouse_input_to_pad(gem_num, inertial_state->pad.digitalbuttons, inertial_state->pad.analog_T);
		}
		else
		{
...
{
	cellGem.fatal("cellGemGetRumble(gem_num=%d, rumble=*0x%x)", gem_num, rumble);
...
s32 cellGemGetState(u32 gem_num, u32 flag, u64 time_parameter, vm::ptr<CellGemState> gem_state)
{
	...
	if (g_cfg.io.move == move_handler::fake)
	{
		if (g_cfg.io.mouse == mouse_handler::basic)
		{
			move::map::mouse_input_to_pad(gem_num, gem_state->pad.digitalbuttons, gem_state->pad.analog_T);
			move::map::mouse_input_to_gem(gem_num, gem_state);
		}
		else
		{
	...
	else if (g_cfg.io.move == move_handler::move)
	{
		...
		move::psmoveapi::tracker::map::gem_state_tracker(handle, gem_state);
	}
...
s32 cellGemInit(vm::cptr<CellGemAttribute> attribute, u32 gem_num)
{
...
{
	cellGem.fatal("cellGemIsTrackableHue(hue=%d)", hue);
	const auto gem = fxm::get<gem_t>();
